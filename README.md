...
| Keen newcomers never break in | Sign up promptly but miss the squad and you bank **one more point than the players who played** — so keen reserves climb *faster* than regulars and break in. |
...
- **Prompt sign-up (miss out on a spot):** the played reward **plus** the prompt bonus (default `+2` played `+1` bonus = **`+3`**) — *one more than the players who played* — for signing up within the prompt window (default 24h of the poll opening) but not making the squad. Softens the disappointment and lets keen reserves climb faster than regulars.
